...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    *************************************************************************/
    ...
    public static void drawBufferedImage(double x, double y, BufferedImage bufferedImg) {
        double xs = scaleX(x);
        double ys = scaleY(y);
        int ws = bufferedImg.getWidth(null);
        int hs = bufferedImg.getHeight(null);
        if (ws < 0 || hs < 0) throw new RuntimeException("image is corrupt");

        offscreen.drawImage(bufferedImg, (int) Math.round(xs - ws/2.0), (int) Math.round(ys - hs/2.0), null);
        draw();
    }
